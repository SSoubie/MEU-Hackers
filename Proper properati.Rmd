...
```{r}
duplicadosbis <- duplicated(properati, incomparables = FALSE)
```

```{r}
summary(duplicadosbis)
```
...
*4º Quitaremos los casos menores a 32 m2 y mayores a 75000m2.* Por qé 32? nuevo código de edificación
...
*OJO, HAY ALGUNOS QUE SE SUPERPONEN ENTRE BARRIOS*

```{r}
properatibarriosunion <- properati2021barrios %>% 
  as.data.frame() %>% 
  select(barrio, promediometro2) %>% 
  rename(BARRIO=barrio)
radiosCensales <- left_join(radiosCensales, properatibarriosunion, by="BARRIO")
```

-------------------------------------------------------------------------------

Para la selección de las manzanas de nuestros interés, construiremos un indice que integre distintas variables.

Puesto que trabajaremos con distintos datasets espaciales, homogeneizaremos los sistemas de coordenadas dos planos. Para ello, utilizaremos el sitema de coordenadas que utiliza el GCBA.
...
properati2021barrios <- st_transform(properati2021barrios, crs = st_crs(hospitales))
radiosCensales <- st_transform(radiosCensales, crs = st_crs(hospitales))
```

Queremos que las manzanas que elijamos se encuentren, como mucho, a 1000 mts de un hospitals público y a 1500 mts de un Csac.
...
2º Hacemos un dataset que solo contenga las avenidas de la CABA.
...
3º Calculamos la distancias de los centroides de manzana a los distintos puntos de transporte
...
*Por qué no directamente al de manzanas?*
...
*Al unir nuestros datasets, vemos que se suman más casos de los que debería haber (12588 frente a 12520).*
...
```{r}
summary(manzanas$promediometro2)
```

```{r}
prueba <- filter(manzanas, cobhospitales==TRUE & cobcesac==TRUE & densidadPob>=16576 & distanciametrobus<=500 & distanciasubte<=500 & distanciaavenida<=200 & promediometro2<=642)
```

```{r fig.height=15, fig.width=15}
ggplot()+geom_sf(data=manzanas, fill="NA")+
        geom_sf(data=prueba, fill="red")+
