...
*Graficamos* *JUSTIFICAR = Completar gráfico*
...
*4º Quitaremos los casos menores a 32 m2 y mayores a 75000m2.* Por qé 32? nuevo código de edificación *JUSTIFICAR*
...
*JUSTIFICACION* *VER ESCALAS* *VALOR M2 o VALOR ALQUILER (M2)*
...
ggplot()+
  ...
  labs(title ="Oficinas y departamentos en alquiler", 
       caption = "Fuente: Properati",
       fill="Valor (m2)")+
  theme_void()
```
...
Por último, unimos nuestro dataset al de radios censales que contiene la información por barrio
...
ggplot()+
  ...
  geom_sf(data=hoteles, color="hotpink4", size=1.5)+
   labs(title = "Cobertura de centros de salud",
...
ggplot()+ geom_sf(data = manzanas, fill = "NA") +
  geom_sf(data = manzanacentroide, color = "lightblue3", size = 0.1)+
...
Hemos decidido considerar las distancias a avenidas, a la red de metrobus y a la red de subte para garantizarla  accesibilidad en diversos medios de transportes
...
ggplot()+
  ...
  labs(title = "Manzanas ideales", 
       subtitle= "Según cercanía a avenidas y las redes de metrobus y subte",
       caption = "Fuente: BA DATA")+
  theme_void()
```
...
Nuestro objetivo es seleccionar aquellas manzanas que cumplan con un determinado parámetro de densidad. Para calcular la misma: 
...
ggplot()+geom_sf(data=manzanas, fill="NA", color="grey60")+
   ...
   labs(title = "Manzanas ideales para la ubicación de un centros de recepción",
