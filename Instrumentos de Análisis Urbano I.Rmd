...
- Aquellos datasets relacionados a la planta del transporte.
...
- Aquellos datasets ligados a infraestructuras de salud.
...
- Aquel dataset que contiene los alojamientos de la CABA.
...
ggplot() + 
  ...
  geom_sf(data=hoteles, aes(color="Hoteles"), size=2, alpha=0.75)+
  geom_sf(data=hospitales,aes(color="Hospitales"), size=2, alpha=0.75)+
  geom_sf(data=csac, aes(color="Csacs"), size=2, alpha=0.75)+
  geom_sf(data=subte,aes(color="Subte"), size=2, alpha=0.75)+
  geom_sf(data = metrobus, aes(color="Metrobús"), show.legend = "point", size=2, alpha=0.75)+
  scale_colour_manual(values= c("Hoteles"="olivedrab4","Hospitales"="hotpink4","Subte"="salmon", "Csacs"="lightblue4", "Metrobús"="gold2"))+
...
- Aquel dataset que posee el precio de las propiedades en alquiler.
...
A partir de este boxplot, podemos ver que valores extremos sesgan nuestra muestra. Por este motivo, el siguiente paso será quitar todos aquellos casos en los que el valor del m2 supere los $50.000.
...
*Graficamos para ver nuestro resultado parcial.*
...
ggplot()+
  ...
  scale_colour_manual(values= c("Hoteles"="black"))+
...
ggplot()+ 
  ...
  geom_sf(data = manzanacentroide, aes (color = "Centroide de Manzana"), size=0.1)+
  geom_sf(data = avenidas, aes (color = "Avenidas"))+
  geom_sf(data = subte, aes(color = "Estación de Subte"), size=2)+
  geom_sf(data = metrobus, aes (color = "Parada de Metrobus"), size=2)+
...
En resumen, hemos decidido considerar las distancias a avenidas, a la red de metrobus y a la red de subte para garantizar la accesibilidad en diversos medios de transportes.

A continuación graficaremos las manzanas (Manzanas ideales) que cumplen con las siguientes condiciones:
...
ggplot()+
  ...
  scale_fill_manual(values = c("Manzanas ideales"="olivedrab")) +
...
1º. Unimos los datasets de cobertura de hospitales y Cesacs al de centroides de manzana. 
...
ggplot()+
  ...
  geom_sf(data = manzanasCobSalud, aes (fill="Manzanas ideales"), show.legend =)+
   scale_fill_manual(values = c("Manzanas ideales"= "lightblue2")) +
  labs(title = "Manzanas ideales", 
...
Nuestro objetivo es seleccionar aquellas manzanas donde haya una gran cantidad personas a fin de que los alojamientos estén cerca de donde más se los necesita. Para calcular la misma: 
...
ggplot()+geom_sf(data=manzanas, fill="NA", color="gray60", size=0.2)+
  ...
  theme(plot.title = element_text(size = 16, hjust = 0.1, vjust = 0.1, face = "bold"),
      plot.caption = element_text(size=10),
      plot.subtitle = element_text(size = 14, hjust = 0.031))
```

**Conclusión:**Al analizar el mapa, se puede observar que las manzanas que cumplen con las condiciones estipuladas son escasas y se encuentran concentradas geograficamente. Esto va en detrimento de la escala que se quiere alcanzar para alivianar el sistema de salud. Por ello, es necesaria una nueva manera de abordar nuestras condiciones.

## **TERCERA PARTE: CONSTRUYENDO UN ÍNDICE**

Entendiendo que los parámetros que seleccionamos en este trabajo pueden ser valorados de distinta manera, hemos decidido construir un índice que pondere nuestras variables.
 
Justamente, antes de avanzar en esta dirección, debemos realizar algunas operaciones sobre las variables previamente mencionadas.
...
Ahora desarrollaremos el índice (Índice 01), considerando las siguientes ponderaciones sobre las variables:

- **Cobertura hospitales: 30%**. Que nuestra manzana se encuentre en las inmediaciones de un hospital es el factor más relevante a tener en cuenta. Ello se debe a que, en el caso de que a un paciente vea agravado su cuadro clínico, se encuentre cerca de un centro de salud y pueda ser trasladado de forma inmediata.
...
-**Distancia avenidas: 15%**, con el fin de facilitar la accesibilidad a los centros de salud por parte de personal que atiende a los pacientes y a las ambulancias.
...
*Graficamos aquellas manzanas que obtuvieron los valores más altos del índice 01*
...
ggplot() +
  ...
  labs(title="Índice 01",
       ...
       caption= "Fuente: BA DATA & Properati") +
  theme_void()+
   theme(plot.title = element_text(size = 18, hjust = 0.1, vjust = 0.3, face = "bold"),
      plot.caption = element_text(size=10),
      plot.subtitle = element_text(size = 12, hjust = 0.1))
```
...
Por otra parte, en violeta se muestra los alojamientos, a fin de ver su ubicación en relación a las manzanas selectas.
...
ggplot() +
  ...
  labs(title="Índice 02",
     subtitle="Manzanas",
      caption= "Fuente: BA DATA & Properati")+
  theme_void()+
    theme(plot.title = element_text(size = 18, hjust = 0.1, vjust = 0.2, face = "bold"),
      plot.caption = element_text(size=10),
      plot.subtitle = element_text(size = 12, hjust = 0.1))
```
...
ggplot()+
  geom_sf(data=manzanas, fill=NA, color="grey60") +
  geom_sf(data=hotelesI2, aes(color="Hoteles"))+
  scale_colour_manual(values = c("Hoteles" ="hotpink4")) +
...
ggplot()+
  ...
  scale_fill_manual(values = c("Manzanas Índice 01" = "blue", "Manzanas Índice 02"="red"))+
   labs(title="Manzanas selectas de los Índices 01 y 02",
       subtitle="Ciudad Autónoma de Buenos Aires",
       fill="Referencias",
       caption= "Fuente: BA DATA & Properati")+
  theme_void()+
...
Frente a esta situación, hemos decidido tomar ambos índices y quedarnos solo con las manzanas, *ergo*, los alojamientos comunes entre ambos. Para ello:
...
ggplot()+
  ...
  geom_sf(data=manzanasindices, aes(fill="Manzana selecta"))+
  geom_sf(data=hotelesindices, aes(color = "Hotel selecto"))+
  geom_sf(data=detectAR, aes(color="DetectAR"), size=2)+
  scale_fill_manual(values = c("Manzana selecta" = "turquoise"), name=NULL)+
  ...
  labs(title="Ubicación centros DetectAR",
       ...
       caption= "Fuente: BA DATA & Properati")+
  theme_void()+
    theme(plot.title = element_text(size=16, hjust=0.07, face = "bold"),
      plot.caption = element_text(size=10),
      plot.subtitle = element_text(size = 12, hjust = 0.07))
```

3º. Estableceremos los radios desde los que se puede llegar caminando desde un detectAR en 15 minutos o menos.
...
*Graficamos para ver los polígonos*
...
4º. Al datasets de distancias caminadas en 15 min. atribuimos el mismo sistema de coordenadas que utilizamos en el resto de los datasets espaciales de este trabajo.
...
5º. Trasnformamos nuestra lista en un objeto espacial que englobe a todos los radios.
...
6º. Buscamos las manzanas que se encuentran a 15 min. o menos de un detectAR.
...
```{r fig.height=10, fig.width=10}
ggplot()+
  geom_sf(data=filter(manzanasdetectAR, cobertura=="FALSE"), aes(fill="Manzanas a más de '15"), color=NA)+
  geom_sf(data=filter(manzanasdetectAR, cobertura=="TRUE"), aes(fill="Manzanas a '15 min o menos"), color=NA)+
  scale_fill_manual(values = c("Manzanas a más de '15" = "blue", "Manzanas a '15 min o menos" = "red"))+
  labs(title="Cobertura centros DetectAR",
       subtitle="a pie",
       fill="Referencias",
       caption= "Fuente: BA DATA")+
  theme_void()+
...
Dentro de estas manzanas, buscamos las manzanas selectas producto (intersección entre índices 01 y 02) que se enceuntren al iterior de dicho rango.
...
Ahora, buscamos los hoteles selectos (de la combinación de los índices 01 y 02) que estén a 15 min o menos de un detectAR.
...
ggplot()+
  ...
  scale_fill_manual(values = c("Manzana selecta (índice 01 y 02)" = "turquoise", "Manzana selecta con cercanía DetectAR"="olivedrab4","Polígonos DetectAR"="orange"))+
  scale_colour_manual(values=c("Hotel selecto con cercanía DetectAR"="salmon", "Polígonos DetectAR"="red")) +
  labs(title="¿Dónde podemos ubicar nuestros alojamientos para mejorar la capacidad de atención sanitaria de la CABA?",
       subtitle="Según distancias caminables en 15 min o menos desde un detectAR ",
       fill="",
       color="Referencias",
       caption= "Fuente: BA DATA & Properati")+
  theme_void()+
...
```{r}
nrow(manzanasindicesdetectAR)
```

```{r}
hotelesindicesdetectar %>% select(2:3) %>% 
head(66)
```
...
A partir del análisis de las bases de datos de BA Data y Properati, podemos concluir que las manzanas de mayor viabilidad para el alquiler de algún inmueble, de acuerdo con los paraámetros establecidos a lo largo del trabajo, son las "manzanas selectas con cercanía DetectAR" (graficadas en el último mapa en color oliva), las que se contabilizan en 549. 
