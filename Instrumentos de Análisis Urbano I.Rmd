...
```{r}
properatiprueba <- st_join(mapaCABA,properatisinNA)
```

```{r}
properatibarrioprueba <-properatiprueba %>% 
  group_by(barrio) %>% 
...
ggplot() +
  geom_sf(data= mapaCABA)+
  geom_sf (data=properatibarrioprueba, aes(fill=promediometro2)) +
...
*Tenemos que averiguar si el centroide se encuentra dentro del rango de cobertura*
...
```{r}
manzanacentroide <- st_join(manzanacentroide, coberturmetrobus)
```

```{r}
manzanacentroide <- st_join(manzanacentroide, coberturasubte)
```
...
```{r}
manzanacentroide <- rename(manzanacentroide, "cobhospitales"=cobertura.x...14,
                           "cobmetrobus"=cobertura.y...15,
                           "cobsubte"=cobertura.x...16,
                           "cobcesac"=cobertura.y...17)
```
...
```{r}
summary(manzanacentroide$densidadPob)
```

```{r}
manzanacentroide <- manzanacentroide %>% 
                      as.data.frame() %>%
                      select(SM,densidadPob,cobhospitales, cobcesac, cobsubte, cobmetrobus)
...
```{r}
manzanas <- mutate(manzanas, cat_distanciavenida=cut(-avenidacercana,breaks = quantile(-avenidacercana,quiebres,na.rm = TRUE ),include.lowest = TRUE))
```

```{r}
prueba <- filter(manzanacentroide, cobhospitales==TRUE & cobmetrobus==TRUE & cobsubte==TRUE & cobcesac==TRUE & densidadPob>=16760)
```

```{r}
ggplot()+geom_sf(data=manzanas)+
        geom_sf(data=prueba, color="red", size=0.01)+
  geom_sf(data=hoteles, color="green")+
        theme_void()
```
...
Ahora vamos a hacer el índice, pero tenemos que sumar, cuando podamos resolver, las siguientes variables: properati y avenidas cercana. 
...
```{r}
manzanas <- manzanas %>%
            mutate(IG=cat_densidad*0.2+cat_distanciasubte*0.15+cat_distanciametrobus*0.15+cat_cobhospitales*0.35+cat_cobcesac*0.15,
                   IG=ifelse(IG>quantile(IG,probs = 0.9,na.rm = TRUE),TRUE,FALSE))
```
...
ggplot() +
  geom_sf(data=mapaCABA) +
  geom_sf(data=manzanas %>% filter(!is.na(IG)) ,aes(fill=IG), color=NA) +
  geom_sf(data=hoteles, color="green", size=1)+
  scale_fill_manual(values = c(NA,"#f03b20")) +
  guides(fill=FALSE) +
  theme_minimal() +
  coord_sf(datum=NA) +
  labs(title="Índice General",
       subtitle="Ciudad de Buenos Aires")
```

Este mapa nos muestra en color rojo las manzanas en las cuales sería correcto colocar los hoteles considerando las siguientes variables: densidad poblacional, distancia subte, distancia metrobus, cobertura hospital y cobertura Csac. 

Ahora bien, en verde, se muestran los alojamientos. Para poder apreciar de forma más clara cuáles serían los alojamientos que se encuentran en dichas manzanas, haremos lo siguiente: 
...
ggplot() +
  geom_sf(data=mapaCABA) +
  geom_sf(data=manzanas %>% filter(!is.na(IG)) ,aes(fill=IG), color=NA) +
  geom_sf(data=coberturahotel, color="green", size=1)+
  scale_fill_manual(values = c(NA,"#f03b20")) +
  guides(fill=FALSE) +
  theme_minimal() +
  coord_sf(datum=NA) +
...
```{r}
manzanas2 <- st_join(manzanas, coberturahotel)
```


```{r}
manzanas3 <- filter(manzanas2, IG=="TRUE" & cobertura=="TRUE")
```
...
ggplot() +
  geom_sf(data=mapaCABA) +
  geom_sf(data=manzanas3, fill="blue") +
  guides(fill=FALSE) +
  theme_minimal() +
  coord_sf(datum=NA) +
  labs(title="Índice General",
...
```{r}
manzanacentroide3 <- st_centroid(manzanas3)
```
...
```
```{r}
manzanacentroide3 <- st_transform(manzanacentroide3,crs = st_crs(detectar))
```

```{r}
detectar <- mutate(detectar, direccionCABA=paste(direccion, "Ciudad de Buenos Aires, Argentina", sep=", "))
```

```{r}
set_key("pcWH0YNYi1G1eqMvAM7fjHWMvqNOaKL6yprw1aMNCeU")
ubicaciones_detectar <- geocode(detectar %>% pull(direccionCABA))
```


```{r}
ubicaciondetectar <- select(detectar, geometry)
```


```{r}
ubicacioncentroidem3 <- select(manzanacentroide3, geometry)
```

```{r}
hoteles <- st_transform(hoteles,crs = st_crs(detectar))
```

```{r}
hoteles <- mutate(hoteles, direccionCABA=paste(direccion, "CIUDAD DE BUENOS AIRES, ARGENTINA", sep=", "))
```

```{r}
set_key("pcWH0YNYi1G1eqMvAM7fjHWMvqNOaKL6yprw1aMNCeU")
ubicaciones_hoteles <- geocode(hoteles %>% pull(direccionCABA))
```


```{r}
viaje <- route(origin = ubicaciones_detectar,
               destination = ubicaciones_hoteles,
               mode = "pedestrian",range=60*15)
```
